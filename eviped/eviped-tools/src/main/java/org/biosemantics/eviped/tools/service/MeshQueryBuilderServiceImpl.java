...
public class MeshQueryBuilderServiceImpl implements QueryBuilder {
	...
	private Collection<String> createQueriesForDrug(String drugMeshName) {
		...
		for (String predicate : PREDICATES) {
			StringBuilder stringBuilder = new StringBuilder("(").append(drugMeshName).append("/").append(predicate)
					.append(")").append(" ").append(FILTER);
			...
			for (Entry<String, Integer> entry : entriesSortedByValues(map)) {
				if (ctr < maxRows) {
					...
					try {
						...
						List<AbstractText> texts = article.getMedlineCitation().getArticle().getAbstract()
								.getAbstractText();
