...
public class DailymedNotationWriter {
	...
	public DailymedNotationWriter(SesameRepositoryClient repoClient, NotationRepository notationRepository) {
		this.sesameRepositoryClient = repoClient;
		...
		try {
			...
			while (result.hasNext()) {
				...
				if (drugbankId.startsWith("DB") && drugbankId.length() == 7) {
					...
					try {
						drugbankNotation = notationRepository.getByCode(drugbankId);
